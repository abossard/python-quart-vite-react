...
Mostly calculations with one I/O action (llm_judge).
...
Supported criteria types:
  ...
  llm_judge       - the LLM grades the output via a judge prompt
"""
...
async def _eval_llm_judge(
    ...
    try:
        ...
        return CriteriaResult(criteria=criteria, passed=passed, detail=answer[:500])
    except Exception as exc:
        return CriteriaResult(criteria=criteria, passed=False, detail=f"LLM judge error: {exc}")


# ---------------------------------------------------------------------------
# PUBLIC EVALUATOR (mostly calculations)
# ---------------------------------------------------------------------------
...
async def evaluate_run(
    ...
    Returns ordered list of CriteriaResult, one per criterion.
    """
