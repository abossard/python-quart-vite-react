...
export default function UsecaseDemoPage({ definition }) {
  ...
  const selectedTicket = useMemo(() => {
    ...
    return (
      matchingTickets.find((t) => {
        if (!t) return false
        // Match by UUID string
        if (String(t.id) === String(selectedTicketId)) return true
        // Match by incident/INC id (case-insensitive)
        if (t.incident_id && String(t.incident_id).toUpperCase() === String(selectedTicketId).toUpperCase()) return true
        return false
      }) || null
    )
                      ...
                      <>
                        {matchingTicketIds.length === 0 ? (
                          ...
                          matchingTicketIds.map((ticketId, idx) => {
                            const ticket = matchingTickets.find((t) => {
                              if (!t) return false
                              if (String(t.id) === String(ticketId)) return true
                              if (t.incident_id && String(t.incident_id).toUpperCase() === String(ticketId).toUpperCase()) return true
                              return false
                            })
                            ...
                            const isSelected = Boolean(
                              selectedTicket && (
                                ...
                                (selectedTicket.incident_id && ticketId && String(selectedTicket.incident_id).toUpperCase() === String(ticketId).toUpperCase())
                              )
                            )
                            return (
                              <tr 
                                ...
                                onClick={() => isLoaded && setSelectedTicketId(ticket?.id || ticketId)}
                                style={{ 
