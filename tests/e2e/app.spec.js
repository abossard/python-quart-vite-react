...
async function waitForAppToLoad(page) {
  ...
  await page.waitForSelector('text=Quart + React Demo Application', { timeout: 15000 })
}
...
test.describe('Task Management - Getting stuff done', () => {
  ...
  test('completes a task with satisfaction', async ({ page }) => {
    ...
    const taskRow = page.locator('[data-testid^="task-title-"]', { hasText: taskTitle })
    const taskId = (await taskRow.getAttribute('data-testid')).replace('task-title-', '')
  ...
  test('edits a task like a pro', async ({ page }) => {
    ...
    const taskRow = page.locator('[data-testid^="task-title-"]', { hasText: originalTitle })
    const taskId = (await taskRow.getAttribute('data-testid')).replace('task-title-', '')
  ...
  test('deletes a task when no longer needed', async ({ page }) => {
    ...
    await page.getByTestId('create-task-button').click()
    const taskTitle = 'Task to be deleted'
    await page.getByTestId('task-title-input').fill(taskTitle)
    await page.getByTestId('save-button').click()
  ...
  test('filters tasks by status like a filtering ninja', async ({ page }) => {
    ...
    await page.getByTestId('create-task-button').click()
    await page.getByTestId('task-title-input').fill('Completed task')
    await page.getByTestId('save-button').click()

    const completedTaskRow = page.locator('[data-testid^="task-title-"]', {
      hasText: 'Completed task',
    })
    const completedId = (await completedTaskRow.getAttribute('data-testid')).replace(
      'task-title-',
      ''
    )
    ...
    await page.getByTestId('create-task-button').click()
    await page.getByTestId('task-title-input').fill('Pending task')
    await page.getByTestId('save-button').click()
...
test.describe('Navigation - Exploring the app', () => {
  ...
  test('shows the awesome project title', async ({ page }) => {
    ...
    await expect(page.getByText('Quart + React Demo Application')).toBeVisible()
  })
})
